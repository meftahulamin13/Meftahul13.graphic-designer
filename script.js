...
if (reviewForm && typeof db !== 'undefined') {
  ...
  // Real-time listener — computes real rating stats from Firestore (Facebook-like counter, no individual cards shown)
  // Base: the 4 fixed sample testimonials shown on the page, all 5-star
  const BASE_COUNT = 4;
  const BASE_RATING_SUM = 20; // 4 reviews x 5 stars
  const BASE_FIVE_STAR_COUNT = 4;

  db.collection('reviews')
    .where('approved', '==', true)
    .onSnapshot((snapshot) => {
      let liveSum = 0;
      let liveFiveStarCount = 0;
      let liveCount = 0;
      ...
      snapshot.forEach((doc) => {
        ...
        liveSum += stars;
        if (stars === 5) liveFiveStarCount++;
        liveCount++;
      });

      const totalCount = BASE_COUNT + liveCount;
      const totalSum = BASE_RATING_SUM + liveSum;
      const totalFiveStar = BASE_FIVE_STAR_COUNT + liveFiveStarCount;
      const avg = totalCount > 0 ? (totalSum / totalCount) : 5;
      const fiveStarPct = totalCount > 0 ? Math.round((totalFiveStar / totalCount) * 100) : 100;

      // "Happy Reviews" counter (46 base marketing number + real submitted reviews)
      const countEl = document.getElementById('reviewCountNumber');
      if (countEl) countEl.textContent = (46 + liveCount) + '+';

      // Average rating stat
      const avgEl = document.getElementById('avgRatingNumber');
      if (avgEl) avgEl.textContent = avg.toFixed(1) + ' ★';

      // 5-star percentage stat
      const fiveStarEl = document.getElementById('fiveStarPercent');
      if (fiveStarEl) fiveStarEl.textContent = fiveStarPct + '%';

      // Big star widget + ratings count
      const bigStarsEl = document.getElementById('bigStars');
      if (bigStarsEl) {
        const rounded = Math.round(avg);
        bigStarsEl.textContent = '★'.repeat(rounded) + '☆'.repeat(5 - rounded);
      }
      const totalRatingsEl = document.getElementById('totalRatingsCount');
      if (totalRatingsEl) totalRatingsEl.textContent = totalCount;
    }, (err) => {
